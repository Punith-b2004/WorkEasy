...
<?php
 ...
 if(isset($_POST['sign'])) {
	 ...
	 if(!empty($name) && !empty($email) && !empty($pass) && !empty($conpass)) {
		 ...
		 $check_query = "SELECT * FROM lform WHERE email = '$email'";
		 ...
		 if(mysqli_num_rows($check_result) > 0) {
			 ...
			 $query = "INSERT INTO lform (name, email, password, conpassword) VALUES ('$name', '$email', '$pass', '$conpass')";
			 $data = mysqli_query($conn, $query);
			 if($data) {
				 echo "<script>alert('Data inserted into database');</script>";
				 header("Location:ssuccessful.php");
				 exit();
			 } else {
